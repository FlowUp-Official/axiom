...
/// Generate a Rust module with serde structs and `validate` methods.
pub fn generate_rust(catalog: &TableCatalog) -> String {
    ...
    emit_preset_helpers(&mut out, catalog);

    if regex_used(catalog) {
        out.push('\n');
...
fn emit_preset_helpers(out: &mut String, catalog: &TableCatalog) {
    let used = used_presets(catalog);
...
fn used_presets(catalog: &TableCatalog) -> Vec<&'static str> {
...
fn regex_used(catalog: &TableCatalog) -> bool {
...
mod tests {
    ...
    fn emits_struct_with_serde_derive() {
        ...
        let out = generate_rust(&TableCatalog { tables: vec![t] });
    ...
    fn emits_regex_matcher_only_when_regex_rules_exist() {
        ...
        let out = generate_rust(&TableCatalog { tables: vec![plain] });
        ...
        let out = generate_rust(&TableCatalog { tables: vec![regex] });
        assert!(out.contains("fn regex_is_match"));
