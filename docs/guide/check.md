...
| `.axm` resolution | Model files parse; imports resolve; no duplicate names; no duplicate field names within a model; no import cycles; `.regex(...)` patterns compile; every query and transaction placeholder is declared; `$param.field` paths address a real model field; validation rules match their field type (numeric rules on numbers, string rules on strings, `nonempty`/`min_length`/`max_length` on strings or arrays, transforms on strings) | `check.axm-parse`, `check.model-resolution`, `check.duplicate-model`, `check.duplicate-field`, `check.import-cycle`, `check.regex-invalid`, `check.query-placeholder`, `check.rule-base`, `check.model` |
| Model sources | `model X infers select|insert|update|delete<relation>` names a real table: exact-case match for qualified names, last-segment match for unqualified names | `check.model-source` |
| Target references | Every model referenced by a declaration emitted for a codegen target is also emitted for that target (its `@target` includes it); model fields, model aliases, and query parameters/return types are checked | `check.target-excluded-reference` |
| Query/Transaction ↔ schema | Query and transaction bodies parse; referenced tables and columns exist; return types resolve to a table, model, or model alias; the SQL body honors the declared return contract | `check.query-sql`, `check.missing-table`, `check.missing-column`, `check.query-return-type`, `check.query-contract` |
