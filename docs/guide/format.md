...
- Model decorators (`@target(...)`, `@no_codegen`, `@parse`,
  ...
  (`@target('rust', 'typescript')` formats as `@target("rust", "typescript")`),
  and `@safeParse` modes are canonicalized to double-quoted
  (`@safeParse(first)` formats as `@safeParse("first")`).
