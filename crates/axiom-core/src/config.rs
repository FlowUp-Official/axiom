...
/// Canonical location of the JSON schema for a released Axiom version.
///
/// The schema is generated from the exact release binary and published as a
/// GitHub Release artifact, so the URL is immutable per version:
///
/// `https://github.com/FlowUp-Official/axiom/releases/download/v<version>/axiom.schema.json`
///
/// Only release builds (built with `AXIOM_RELEASE=1`, as in the GitHub Actions
/// release workflow) embed this URL. Development builds omit `$schema`
/// entirely: their `CARGO_PKG_VERSION` may not correspond to a published
/// release yet, so emitting a version-pinned URL could point at a schema
/// generated by different code than the running binary.
pub const SCHEMA_VERSION_URL: Option<&str> = match option_env!("AXIOM_RELEASE") {
    Some(_) => Some(concat!(
        "https://github.com/FlowUp-Official/axiom/releases/download/v",
...
/// Build the version-pinned, release-asset schema URL for `version`.
pub fn schema_version_url(version: &str) -> String {
    format!("https://github.com/FlowUp-Official/axiom/releases/download/v{version}/axiom.schema.json")
}
...
pub struct AxiomConfig {
    /// URL pointing to the Axiom JSON schema for this release, e.g.
    /// `https://github.com/FlowUp-Official/axiom/releases/download/v0.7.0/axiom.schema.json`.
...
impl AxiomConfig {
    ...
    pub fn default_template() -> Self {
        ...
        Self {
            schema: SCHEMA_VERSION_URL.map(|url| url.to_string()),
...
mod tests {
    ...
    fn default_template_is_valid_axiom_config() {
        ...
        match option_env!("AXIOM_RELEASE") {
            Some(_) => {
                let expected = SCHEMA_VERSION_URL
                    .expect("release builds embed the version-pinned schema URL");
                assert_eq!(schema, Some(expected));
                assert!(
                    !expected.contains("/main/"),
                    "schema URL must pin the release version, not main: {expected}"
                );
            }
            None => assert_eq!(
                schema,
                None,
                "development builds must omit `$schema`: the version may not correspond to a published release"
            ),
        }
    }

    #[test]
    fn schema_url_pins_cargo_package_version_in_release_builds() {
        let expected = schema_version_url(env!("CARGO_PKG_VERSION"));
        assert_eq!(
            expected,
            format!(
                "https://github.com/FlowUp-Official/axiom/releases/download/v{}/axiom.schema.json",
                env!("CARGO_PKG_VERSION")
            )
        );
        ...
        match option_env!("AXIOM_RELEASE") {
            Some(_) => assert_eq!(
                ...
                "release builds must pin exactly the version that produced the binary"
            ),
            None => assert_eq!(
                SCHEMA_VERSION_URL,
                None,
                "development builds must not emit a version-pinned schema URL"
            ),
        }
    }

    #[test]
    fn schema_url_uses_release_asset_form() {
        assert_eq!(
            schema_version_url("1.2.3"),
            "https://github.com/FlowUp-Official/axiom/releases/download/v1.2.3/axiom.schema.json"
        );
        assert_eq!(
            schema_version_url("0.7.0-rc.1"),
            "https://github.com/FlowUp-Official/axiom/releases/download/v0.7.0-rc.1/axiom.schema.json"
        );
    }
